...
list<string>* playWordGame(list<string> * dictionary, string& initialWord, string& targetWord)
{
	...
	priority_queue<WordNode> * fringe;
	WordNode initialWordNode(initialWord, 100);
	fringe->push(initialWordNode);
	...
	WordNode currentWordNode(currentWord);
	...
	while(currentWordNode.getWord() != targetWord)
	{
		...
		{
			char currentChar = (currentWordNode.getWord())[i];
			for(int j = 0; j < 26; j++)
			{
				...
				if((currentWordNode.getWord())[i] != alphabet[j])
				...
				if(foundInDictionary != dictionary->end())
				{
					...
					if(foundInExplored == exploredWords->end())
					{
						...
						WordNode foundWordNode(currentWord);
						foundWordNode.calculateHeuristic(targetWord);
						cout << "Heuristic Value of word " << currentWord << ": " << foundWordNode.getHeuristic();

						// The word exists in the dictionary and should be added to the fringe.
						fringe->push(foundWordNode);
					}
				}
				cout << "Oopsie woopsie! you made a fucky wucky with that word! OwO >:3n TWY AGANE!!!" << endl;
			}
		...
		currentWord = (fringe->top()).getWord();
	}
	if(currentWord == targetWord)
